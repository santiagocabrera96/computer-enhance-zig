...
pub inline fn timeBlockStart(name: []const u8) struct { index: u64, start: u64, parent_index: u64, old_tsc_elapsed_At_root: u64 } {
    const I = struct {
        var idx: u64 = 0;
    };
    ...
    return .{ .index = I.idx, .start = start, .parent_index = parent_index, .old_tsc_elapsed_At_root = anchor.tsc_elapsed_at_root };
}
...
pub fn timeBlockEnd(block: anytype) void {
    ...
    anchor.tsc_elapsed_at_root = elapsed + block.old_tsc_elapsed_At_root;
    anchor.hit_count += 1;
}
    ...
    for (global_profiler.anchors) |anchor| {
        if (anchor.tsc_elapsed > 0) {
            ...
            if (anchor.tsc_elapsed_at_root != elapsed) {
                const percent_elapsed_with_children = @as(f64, @floatFromInt(anchor.tsc_elapsed_at_root)) * 100 / total_elapsed_f64;
