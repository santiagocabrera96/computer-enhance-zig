...
const parseHaversinePairs = @import("listing_0069_lookup_json_parser.zig").parseHaversinePairs;
...
pub fn simpleHaversineMain(allocator: std.mem.Allocator, input_json_filename: []const u8, answers_filename: ?[]const u8) !void {
    const total_start = timer.readCPUTimer();

    const read_start = total_start;
    const input_json = try readEntireFile(allocator, input_json_filename);
    defer allocator.free(input_json);

    const misc_setup_start = timer.readCPUTimer();
    ...
    const parse_start = timer.readCPUTimer();
    ...
    const misc_output_start = timer.readCPUTimer();
    ...
    const total_end = timer.readCPUTimer();
    const total_elapsed = total_end - total_start;

    const timer_freq = timer.estimateCPUTimerFreq();

    const total_elapsed_time_ms: f64 = @as(f64, @floatFromInt(total_elapsed)) * (1000 / @as(f64, @floatFromInt(timer_freq)));
    std.log.info("Total time: {d:.4}ms (CPU freq {}) elapsed {d}", .{ total_elapsed_time_ms, timer_freq, total_elapsed });

    const total_elapsed_f64: f64 = @floatFromInt(total_elapsed);

    std.log.info("  {s}: {} ({d:.2}%)", .{ "Read", misc_setup_start - read_start, @as(f64, @floatFromInt(misc_setup_start - read_start)) * 100 / total_elapsed_f64 });
    std.log.info("  {s}: {} ({d:.2}%)", .{ "MiscSetup", parse_start - misc_setup_start, @as(f64, @floatFromInt(parse_start - misc_setup_start)) * 100 / total_elapsed_f64 });
    std.log.info("  {s}: {} ({d:.2}%)", .{ "Parse", sum_start - parse_start, @as(f64, @floatFromInt(sum_start - parse_start)) * 100 / total_elapsed_f64 });
    std.log.info("  {s}: {} ({d:.2}%)", .{ "Sum", misc_output_start - sum_start, @as(f64, @floatFromInt(misc_output_start - sum_start)) * 100 / total_elapsed_f64 });
    std.log.info("  {s}: {} ({d:.2}%)", .{ "MiscOutput", total_end - misc_output_start, @as(f64, @floatFromInt(total_end - misc_output_start)) * 100 / total_elapsed_f64 });
